...
@ObservabilityRegistry.register(ModuleMetadata(name="memory_plotter", version="1.0", schema_version="v1", category="plotter"))
class MemoryPlotter(ObservabilityModule):
    def __init__(self):
    ...
    def export(self, session_id: str, observations: List[Observation]) -> None:
        metrics = [o for o in observations if isinstance(o, MetricObservation) and "memory" in o.name]
        if not metrics:
            return
        ...
        plt.savefig(os.path.join(self.output_dir, f"session_{session_id}_memory.png"))
        plt.close()
