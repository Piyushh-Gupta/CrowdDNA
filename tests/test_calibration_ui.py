...
def test_get_first_frame_no_video():
    img, df, msg, img2, state_pts, en, calib_st, calib_msg, calib_prev = get_first_frame(None)
    assert img is None
...
def test_handle_image_click_ordering():
    ...
    img1, s1, m1, _, _, _, _ = handle_image_click(Evt((10, 10)), img, state)
    assert s1 == [(10, 10)]

    img2, s2, m2, _, _, _, _ = handle_image_click(Evt((90, 10)), img, s1)
    img3, s3, m3, _, _, _, _ = handle_image_click(Evt((90, 90)), img, s2)
    img4, s4, m4, _, _, _, _ = handle_image_click(Evt((10, 90)), img, s3)
    ...
    img5, s5, m5, _, _, _, _ = handle_image_click(Evt((50, 50)), img, s4)
    assert len(s5) == 1
    assert s5 == [(50, 50)]
...
def test_image_point_change_invalidates_calibration():
    import numpy as np
    img = np.zeros((100, 100, 3), dtype=np.uint8)
    state = [(0, 0), (10, 0), (10, 10)]

    class Evt:
        def __init__(self, idx):
            self.index = idx

    img_out, state_out, msg, calib_state, val_msg, prev, enabled = handle_image_click(Evt((0, 10)), img, state)

    assert calib_state == "UNVERIFIED / INCOMPLETE"
    assert val_msg == ""
    assert prev == ""
